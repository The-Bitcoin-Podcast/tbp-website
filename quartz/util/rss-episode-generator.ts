...
export class RSSEpisodeGenerator implements RSSEpisodeGeneratorContract {
  ...
  async generateEpisodeFile(
    ...
    try {
      ...
      const filePath = generateEpisodeFilePath(config.outputDirectory, config.podcastFolder, filename)
      ...
      if (config.includeAudioEmbed && episode.enclosureUrl) {
        ...
        contentSections.push(`  <source src="${episode.enclosureUrl}" type="${episode.enclosureType}">`)
      ...
      contentSections.push('')
      contentSections.push('- [Spotify](https://open.spotify.com/show/6kXFmD6rcYPgh0h70sRBha)')
      contentSections.push('- [Apple Podcasts](https://podcasts.apple.com/us/podcast/hashing-it-out/id1520467946)')
